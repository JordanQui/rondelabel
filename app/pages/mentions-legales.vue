<template>
  <main class="legal-page">
    <div class="legal-container">
      <h1>Mentions légales</h1>

      <h2>Éditeur du site</h2>
      <p>
        Ce site est édité par :<br />
        <strong>Jordan Quiqueret</strong> (nom civil),<br />
        agissant sous le nom d’artiste <strong>Jordan Olivier</strong>.<br />
        Courriel de contact :
        <a href="mailto:contact@jordanolivier.com">jordanxyolivier@gmail.com</a>
      </p>

      <h2>Hébergement</h2>
      <p>
        Hébergeur : <strong>OVH SAS</strong><br />
        2 rue Kellermann — 59100 Roubaix — France<br />
        Site :
        <a href="https://www.ovh.com" rel="noopener noreferrer" target="_blank">www.ovh.com</a>
      </p>

      <h2>Propriété intellectuelle</h2>
      <p>
        Tous les contenus présents sur ce site (textes, musiques, images, vidéos, code, etc.) sont protégés par le
        droit d’auteur.<br />
        © 2025 <strong>Jordan Olivier</strong> — Tous droits réservés.<br />
        Diffusion sous licence
        <a
          href="https://creativecommons.org/licenses/by-nc-nd/4.0/deed.fr"
          rel="noopener noreferrer"
          target="_blank"
        >
          Creative Commons BY-NC-ND 4.0 International
        </a>
        .<br />
        Toute utilisation commerciale ou modification est interdite sans autorisation écrite.
      </p>

      <h2>Responsabilité</h2>
      <p>
        L’éditeur ne saurait être tenu pour responsable d’un dommage direct ou indirect résultant de l’accès ou de
        l’utilisation du site.<br />
        Les liens externes proposés ne constituent pas une approbation de leur contenu.
      </p>

      <h2>Données personnelles</h2>
      <p>
        Ce site ne collecte aucune donnée personnelle sans le consentement explicite de l’utilisateur. Aucune
        information n’est transmise à des tiers.
      </p>
    </div>
  </main>
</template>

<script setup lang="ts">
useHead({
  title: 'Mentions légales - Jordan Olivier'
});
</script>
