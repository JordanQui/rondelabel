      ...
      <div ref="sliderTrackRef" class="slider-track">
        ...
        >
          <div class="slide-information">
            <p class="mix-name">{{ track.title }}</p>
            <p class="track-name">Jordan Olivier</p>
          </div>
...
function goToSlide(
  ...
  handleHydraOscSlideChange(previousIndex, targetIndex)
...
function animateHydraOscOffset(target: number) {
  if (hydraOscOffsetTween) {
    hydraOscOffsetTween.kill()
    hydraOscOffsetTween = null
  }

  const localGsap = sliderGsap ?? (typeof window !== 'undefined' ? gsap : null)
  if (!localGsap) {
    hydraOscOffset.value = target
    return
  }

  hydraOscOffsetTween = localGsap.to(hydraOscOffset, {
    value: target,
    duration: HYDRA_OSC_TRANSITION_DURATION,
    ease: 'none',
    onComplete: () => {
      hydraOscOffsetTween = null
    },
...
function handleHydraOscSlideChange(previousIndex: number, nextIndex: number) {
  if (previousIndex === nextIndex) {
    return
  }

  const target = HYDRA_OSC_BASE_OFFSET + nextIndex
...
async function initHydra() {
  ...
  osc(hydraBand('lowMid',1,15),0,() => hydraOscOffset.value + hydraOscAnimation.value)
    ...
    .modulateRotate(o0, () => hydraBand('high',0,1) + 1)
    .scale(1)
    .saturate(1.35)
    .contrast(1.08)
    .modulate(o0, ()=> hydraBand('highMid',1,10))
    .modulate(o0, ()=> hydraBand('highMid',1,10))
    .modulate(o0, ()=> hydraBand('highMid',1,10))
    .color(() => hydraBand('low',0, 2),() => hydraBand('lowMid',0, 2),() => hydraBand('high',0, 2))
    .color(() => hydraBand('low',0, 2),() => hydraBand('lowMid',0, 2),() => hydraBand('high',0, 2))
    .color(() => hydraBand('low',0, 2),() => hydraBand('lowMid',0, 2),() => hydraBand('high',0, 2))
    .out(o0)
}
